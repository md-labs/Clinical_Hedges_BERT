...
class InputProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, i, task_num):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "Train_Data_Part_{}_Answer.tsv".format(i, task_num))), "train")

    def get_dev_examples(self, data_dir, i, task_num):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "Dev_Data_Part_{}_Answer.tsv".format(i, task_num))), "dev")

    def get_test_examples(self, data_dir, i, task_num):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "Test_Data_Part_{}_Answer.tsv".format(i, task_num))), "test")
...
def main():
    ...
    for part_index in range(args.num_parts_start, args.num_parts_end):
        ...
        if args.do_train:
            train_examples = processor.get_train_examples(args.data_dir, part_index, task_num)
        ...
        if args.do_train:
            ...
            logger.info("***** Running training on Part {}  Task {}*****".format(part_index, task_num))
            ...
            for ep in trange(int(args.num_train_epochs), desc="Epoch"):
                ...
                eval_examples = processor.get_dev_examples(args.data_dir, part_index, task_num)
        ...
        if args.do_train:
            ...
            if(os.path.exists(os.path.join(args.output_dir, "Model_Part_{}_Task_{}".format(part_index, task_num)))):
                shutil.rmtree(os.path.join(args.output_dir, "Model_Part_{}_Task_{}".format(part_index, task_num)))
            os.mkdir(os.path.join(args.output_dir, "Model_Part_{}_Task_{}".format(part_index, task_num)))
            output_model_file = os.path.join(args.output_dir, "Model_Part_{}_Task_{}".format(part_index, task_num), WEIGHTS_NAME)
            torch.save(model_to_save.state_dict(), output_model_file)
            output_config_file = os.path.join(args.output_dir, "Model_Part_{}_Task_{}".format(part_index, task_num), CONFIG_NAME)
        ...
        if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            eval_examples = processor.get_test_examples(args.data_dir, part_index, task_num)
            eval_features = convert_examples_to_features(
                eval_examples, label_list, args.max_seq_length, tokenizer)
            complete_user_ids = list()
            for example in eval_examples:
                complete_user_ids.append(example.guid)
            logger.info("***** Running Test for Part {} Task {}*****".format(part_index, task_num))
            ...
            outcsv = open(os.path.join(args.output_dir, "Reqd_Labels_Part_{}_Task_{}.csv".format(part_index, task_num)),'w', encoding = 'utf8', newline='')
